...
{
    public class SecondaryAuthService : ISecondaryAuthService
    {
        private readonly HttpClient _httpClient;
        private readonly AuthenticationStateProvider _authenticationStateProvider;
        private readonly ILocalStorageService _localStorage;

        public SecondaryAuthService(HttpClient httpClient,
        ...
        public async Task<SecondaryRegister.Result> Register(SecondaryRegister.Command registerModel)
        {
            var response = await _httpClient.PostAsJsonAsync("api/secondary/register", registerModel);

            return await response.Content.ReadFromJsonAsync<SecondaryRegister.Result>();
        }
        ...
        public async Task<SecondaryLoginPassword.Result> Login(SecondaryLoginPassword.Command loginModel)
        {
            var response = await _httpClient.PostAsJsonAsync("api/secondary/login", loginModel);
            var loginResult = await response.Content.ReadFromJsonAsync<SecondaryLoginPassword.Result>();
